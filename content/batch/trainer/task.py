...
def train_and_evaluate(
    work_dir, train_df, eval_df, batch_size=1024, epochs=1, steps=1000
):
    ...
    pickle.dump(
        tokenizer,
        tf.io.gfile.GFile(model_dir+TOKENIZER_MODEL, mode="wb"),
        protocol=0,
    )
    with tempfile.NamedTemporaryFile(suffix=".h5") as local_file:
        with tf.io.gfile.GFile(
            model_dir+ KERAS_MODEL, mode="wb"
        ) as gcs_file:
            model.save(local_file.name)
            gcs_file.write(local_file.read())

    word2vec_model.save(model_dir+ WORD2VEC_MODEL)

    pickle.dump(label_encoder, open(model_dir+ ENCODER_MODEL, "wb"), protocol=0)
...
if __name__ == "__main__":
    ...
    train_data_files = tf.io.gfile.glob(args.work_dir+"/transformed_data/train/part-*")
    eval_data_files = tf.io.gfile.glob(args.work_dir+ "/transformed_data/eval/part-*")
